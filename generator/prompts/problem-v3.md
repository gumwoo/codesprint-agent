...
6. **이 Skill 을 쓰지 않고도 풀리면 버려진다.** 출력만 보는 채점은 어떤 자료구조를 썼는지
   보지 못한다. {{skillControlRule}}
...
- **본문에 이 Skill 의 핵심 식이나 알고리즘을 적지 않는다.** 규칙은 말과 예시로 정의한다.
  "idx = r * M + c" 나 "B[j][n-1-i] = A[i][j]" 처럼 답을 구하는 식을 주면 재려던 것이 사라진다.
- **제약의 끝에서도 정답 출력이 1MB 를 넘지 않게** 제약을 정한다. 한 줄에 쓰는 최대 글자 수 ×
  최대 줄 수를 계산해 본다. 넘으면 N · Q 의 상한을 줄인다.
- skillControl 이 있으면 **큰 입력은 다양해야 한다.** 질의 · 갱신 · 값이 서로 달라야 하고, 정렬돼
  있거나 등차수열이거나 모든 값이 같거나 답이 자명한(전부 조건을 만족하는) 입력이면 안 된다. 같은
  질의를 묶어 캐시하거나, 같은 갱신을 묶거나, 조기에 멈추거나, 입력 모양을 이용한 공식으로 푸는
  풀이가 통하면 그 문제는 Skill 을 재지 못한다. 큰 입력 생성기는 고정 seed 의 random 으로 만든다.
- 모든 case 는 statement 의 제약 안이어야 한다.
