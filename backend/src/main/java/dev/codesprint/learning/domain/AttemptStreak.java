 ...
 *   <li><b>풀어낸 뒤의 기록까지 남는다.</b> 두 번 틀리고 결국 맞힌 문제를 나중에
 *       다시 풀다 한 번 틀리면 곧바로 3회째다.
 * </ul>
 *
 * <p>그래서 <b>마지막 정답 이후</b>의, <b>Evidence 를 만드는 실패</b>만 센다.
 *
 * <p><b>끊는 기준은 정답 하나뿐이다.</b> 시간은 보지 않는다 - 6월에 두 번 틀리고
 * 포기했다가 9월에 다시 틀리면 3회로 센다. 지금 규칙이 "마지막 정답 이후의 연속
 * 실패" 이므로 그것이 맞는 동작이다. "이번 학습 구간에서 3회" 로 좁히려면 세션
 * 경계가 필요한데, 그 개념이 아직 없다. 세션이 생기면 여기에 그 경계를 더한다.
 */
