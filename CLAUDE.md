...
PRD 의 웨이브(W1~W9, [ADR-0034](docs/adr/0034-the-road-to-the-full-prd.md))를 마쳤다. 도메인 46 전부 ·
Skill 79. 커리큘럼 데이터, 계약, 하네스, Judge/Sandbox(Python · Java · C++), 검증된 문제 118개, Mastery 산식, 백엔드(Spring Boot · PostgreSQL · Flyway), Decision Engine,
...
화면 검사는 두 층이다([ADR-0025](docs/adr/0025-browser-race-tests.md)).

| | 무엇을 보는가 |
| --- | --- |
| `WebClientTest` | 표(`claimView`)를 받았는가 · 부르는 엔드포인트가 실재하는가 |
| `e2e/` | **늦게 온 응답이 최신 화면을 덮는가** (실제 브라우저) |

늦게 온 응답이 최신 화면을 덮는 결함이 여섯 번 나왔고 전부 사람이 찾았다. 정적
검사는 "표를 받았는가" 까지만 볼 수 있어서, 통을 잘못 나눈 것도 축이 모자란 것도
잡지 못한다.
...
먼저 도착하는 날이 생기고, 그때 테스트는 조용히 통과한다. 새 시나리오를 넣을 때는
...
## 측정할 수 없는 것은 측정하지 않는다

**힌트 사용량을 사용자에게 묻지 않는다.** 힌트 기능이 없는데도 제출 API 가
`hintLevel` 과 `solutionViewed` 를 받고 있었고, Evidence 는 그 값으로 독립 풀이
...
문제 화면에 "막히면" 패널이 있다. 버튼이 다음 한 칸을 열고, **몇 단계까지 봤는지는
서버가 준 값을 그대로 적는다** - 화면이 세기 시작하면 사용자가 보는 값과 채점에
들어가는 값이 갈린다.
...
복사본을 만들지 않는다. CI 가 사다리가 실제로 사다리인지 본다 - 단계가 다 있는지,
두 칸이 붙지 않았는지, 정답 코드를 그대로 담지 않았는지.

힌트를 만들 때도 **클라이언트가 신고하지 않는다.**
...
슬라이스 1 범위는 Python 3.12 + BFS Grid 계열 8개 Skill + Mistake 2종 자동 드릴이었다. 그 뒤 웨이브는
ADR-0034 가 정한다 - 새 도메인은 Skill 정의 · 선수 · 개념 자료 · 문제 · 검증 에이전트 검토를 함께 갖춰야 켜진다.
