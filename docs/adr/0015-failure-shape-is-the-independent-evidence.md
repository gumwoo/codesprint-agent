...
- 태그는 슬라이스 1에서 `BOUNDARY_CHECK`(P02·P03·P04·P09)와 `VISITED_TIMING`(P04)이다.
  자동 드릴이 켜진 두 Mistake가 모두 실패의 모양으로 확정될 수 있다.
- **`VISITED_TIMING`을 붙이려고 case를 하나 설계했다.** 기존 case로는 안 됐다 —
  `INDEX_ERROR`가 실패시키는 case가 `VISITED_TIMING`의 것을 포함해, 어떤 부분집합을
  골라도 구별되지 않았다. 그래서 **대칭 격자**를 넣었다. 행/열을 뒤바꿔 읽어도 같은
  값이라 `INDEX_ERROR`는 드러나지 않고, 2×2 덩어리는 한 칸이 두 이웃에서 각각 큐에
  들어가므로 꺼낼 때 방문 처리하면 그 칸이 두 번 세어진다.
- 태그가 `{4, 7}` 인 이유도 같다. 새 case 하나만으로는 `BOUNDARY_CHECK`도 그것을
  실패시켜 구별되지 않는다. **둘을 함께 요구해야** 세 경쟁 실수 모두와 갈린다.
- `P10_BFS_REVIEW`에는 붙일 수 없었다. `OUTPUT_FORMAT`이 거의 모든 case를 실패시켜
  `VISITED_TIMING`의 실패 집합을 포함한다 — P05·P10에서 `BOUNDARY_CHECK`를 뗀 것과
  같은 이유다.
