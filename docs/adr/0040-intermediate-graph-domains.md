...
| P80 · P81 | 무작위 그래프에서는 우선순위 큐 없이 큐로 완화를 되풀이하는 풀이(SPFA)가 통과한다 | 5 행 격자(가로 간선 무작위, 세로 1) - SPFA 가 정점마다 수백 번 꺼내 TIME_LIMIT. 검토 뒤 10 행 격자를 더했다(아래 남는 위험) |
...
- **다익스트라는 휴리스틱 SPFA 와 채점으로 가를 수 없다.** 검증 에이전트가 우선순위 큐 없는 SPFA 에 SLF(작으면
  앞에) · LLL(평균보다 크면 뒤로)을 붙인 풀이로 P80 · P81 을 AC 했다. 10 행 격자 case 를 더해 SLF 는 P81 에서
  TIME_LIMIT 이 됐지만, P80 은 M ≤ 50000 안에서 SLF 가 약 100 만 번 꺼내고도 시간 안에 끝난다. LLL 은 두
  문제 모두 정답과 비슷한 속도다 - 격자 모양 · 가중치 범위를 바꿔 찾아봤지만 막는 입력을 찾지 못했다.
  DIJKSTRA 의 Evidence 는 "음이 아닌 가중치 그래프에서 최단 거리를 효율적으로 구했다" 까지만 말한다(GREEDY ·
  P72 와 같은 처리). 다이얼 버킷은 가장 가까운 것부터 확정하는 다익스트라라 정의 안이다
- 플로이드 · 위상 순서의 정의는 방법이 아니라 결과로 적는다. 정점마다 다익스트라(P84), DFS 종료 역순(P88)도
  같은 답을 내고, 대조가 없으니 채점으로 가를 수 없다. 처음 정의에 방법을 적었다가 검토에서 고쳤다
  (W4 의 사이클 판정과 같은 모양)
