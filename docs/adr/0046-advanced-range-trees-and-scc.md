...
- **P117 은 차수 0 을 떼고 무작위 피벗으로 나누는 전방-후방 분할(FW-BW)로 AC 다**(샌드박스 1024ms). 기대 시간이 거의
  선형이라 정의("그래프 크기에 비례하는 시간")에 가깝다. 방금 뗀 묶음 근처에서 피벗을 고르는 휴리스틱은 TIME_LIMIT 지만
  제한의 약 1.2 배로 가깝다. 그래서 SCC_DECOMPOSITION 의 Evidence 는 "SCC 를 FW-BW 계열 이상으로 구했다" 까지만 말한다
  (P115 · P118 · P120 의 제곱근 분할과 같은 처리)
