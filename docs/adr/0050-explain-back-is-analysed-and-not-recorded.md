...
| 검사 | 무엇을 보는가 |
| --- | --- |
| `ExplainTest` | 푼 문제 → 200 · 계약 · 프롬프트에 질문과 설명이 그대로 · Evidence 행 수 그대로 / 제출 없음 · 틀리기만 함 → 409(모델을 부르지 않음), 풀면 200(대조) / score 를 붙인 답 502 · 꺼짐 503 · 빈 설명 400 · 시험 중 409, 끝나면 200(대조). "푼 문제만" 확인을 끄면 실패 |
| `ExplainSharesReviewerSettingsTest` | 명령이 Reviewer 와 같고 도구가 없으며 기본이 꺼져 있다, 프롬프트의 자리를 코드가 전부 채운다 |
