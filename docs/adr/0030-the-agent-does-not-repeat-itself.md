...
### "이미 보여줬는가" 는 **건넨 사실**을 본다

처음에는 `submissions.next_action_type` 이 `REVIEW_CONCEPT` 였던 적이 있는지로
판단했다. 리뷰에서 잡혔고, 재현했다.

```
3회 실패 -> REVIEW_CONCEPT 결정
"다음 단계 보기" 를 누르지 않음      <- 자료는 화면에 뜨지 않았다
또 실패
=> 자료를 한 번도 보지 못한 채 다시는 받지 못한다
```

**결정은 결정이고 관측은 관측이다.** `next_action_type` 은 "보여주기로 정했다" 이지
"보여줬다" 가 아니다. 자료가 실제로 화면에 뜨는 것은 사용자가 `next-problem` 을
불렀을 때다.

힌트에서 그은 선과 같다(ADR-0027) — **클라이언트가 신고하지 않고, 서버가 내주면서
남긴다.** "측정할 수 없는 것은 측정하지 않는다" 도 같은 자리다.

그래서 `submissions.concept_delivered_at` 을 두고(V11), 자료가 응답에 실려 나가는
그 자리에서 찍는다. 처음 한 번만 찍으므로 새로고침이 무엇도 늘리지 않는다.

조회인데 쓰기가 있는 것은 의도된 것이다. 힌트를 `POST` 로 둔 것과 다른 선택인데,
그쪽은 사용자가 사다리를 한 칸씩 올리는 **행동**이고 이쪽은 이미 정해진 결정을
옮겨 주는 것이라 셀 것이 없기 때문이다.
...
- **한 번 받은 자료는 그 문제에서 다시 받을 수 없다.** 한참 뒤에 같은 문제로 돌아와도
