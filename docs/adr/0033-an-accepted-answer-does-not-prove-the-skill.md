...
- **P01** 을 고쳤다. 넣은 값으로 줄을 400000 칸 채운 뒤 **넣기와 꺼내기를 번갈아** 90000 번
  ...
  **매번** 옮기게 하고, 값은 **2 바이트 배열에 담기지 않게** 여섯 자리로 둔다. 출력 상한(1MB)
  ...
  | 풀이 (샌드박스 이미지, --cpus=1 --memory=256m, 제한 없이 실측) | 시간 |
  | --- | --- |
  | 정답 (deque) | 501ms |
  | list `pop(0)` (skill_control.py) | 8046ms |
  | `array('i')` 의 `pop(0)` | 3971ms |
...
- **시간으로만 가른다.** 기계가 몇 배 빠르면 `pop(0)` 풀이가 제한 안에 들어올 수 있다.
  P01 에서 가장 가까운 것은 `array('i')` 의 `pop(0)` 으로 제한 대비 2.0배(3971ms / 2000ms)다.
  기계가 두 배 빠르면 들어올 수 있다
