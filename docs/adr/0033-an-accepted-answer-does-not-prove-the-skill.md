...
- **P01** 을 고쳤다. 처음 줄을 1..M 으로 두는 조건을 더하고, **넣기와 꺼내기가 번갈아
  오는** 큰 case 하나(push 200000 번 뒤 (pop, push) 200000 번)를 맨 뒤에 넣었다.
  ...
  | push 를 몰아 넣고 pop 을 몰아 꺼낸다 | 넣기↔꺼내기가 바뀔 때만 list 를 뒤집는다 - 한 번만 옮기고 끝난다 |

  길이는 **넣은 값**으로 만들고, 꺼낼 때마다 옮기는 풀이가 **매번** 옮기게 해야 한다.

  | 풀이 (--cpus=1, 제한 없이 실측) | 시간 |
  | --- | --- |
  | 정답 (deque) | 235ms |
  | 넣은 값을 `pop(0)` | 8512ms |
  | 바뀔 때만 뒤집기 | 29960ms |

  채점에서는 `pop(0)` · `insert(0)` · 뒤집기 풀이 모두 TIME_LIMIT, 앞 위치 번호 풀이는
  ACCEPTED. 출력 상한(1MB)에 맞춰 제약도 고쳤다(M ≤ 30000, 0 ≤ X ≤ 9, pop ≤ 300000) - 제약
  안의 입력에서 정답이 OUTPUT_LIMIT 을 받을 수 있었다. `cases.json` 이 4.8MB 가 됐다 - Worker
  가 호스트에서 읽으므로(judge-job 계약) 큐 행에는 들어가지 않는다
...
- **시간으로만 가른다.** 기계가 몇 배 빠르면 `pop(0)` 풀이가 제한 안에 들어올 수 있다.
  P01 은 제한 대비 4.3배(8512ms / 2000ms)를 두었지만 증명은 아니다
