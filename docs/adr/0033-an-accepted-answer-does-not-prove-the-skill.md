...
`meta_test_adoption.py` 에 여섯 케이스를 더했다. 대조를 갖춘 큐 초안은 채택되고, 대조 풀이가
...
- **P01** 을 고쳤다. 처음 줄을 1..M 으로 두는 조건을 더하고, **넣은 값을 한꺼번에 꺼내는**
  큰 case 하나(push 300000 번 뒤 pop 300000 번, N ≤ 600000)를 맨 뒤에 넣었다.

  첫 시도는 처음 줄 1..M 만 길었다. 검증 에이전트가 "처음 줄은 번호만 세고 넣은 값만 list
  에 두고 `pop(0)`" 하는 풀이로 AC 를 받았다 - 처음 줄은 값이 알려진 연속 번호라 저장할
  필요가 없다. 길이는 **넣은 값**으로 만들어야 한다.
  ...
  채점에서는 두 `pop(0)` 풀이 모두 TIME_LIMIT, 앞 위치 번호 풀이는 ACCEPTED. `cases.json` 이
  4.8MB 가 됐다 - Worker 가 호스트에서 읽으므로(judge-job 계약) 큐 행에는 들어가지 않는다
...
- **시간으로만 가른다.** 기계가 몇 배 빠르면 `pop(0)` 풀이가 제한 안에 들어올 수 있다.
  P01 은 제한 대비 4.5배(9011ms / 2000ms)를 두었지만 증명은 아니다
