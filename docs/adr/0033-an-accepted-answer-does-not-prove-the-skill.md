...
`meta_test_adoption.py` 에 네 케이스를 더했다. 대조를 갖춘 큐 초안은 채택되고, 대조 풀이가
없거나 · 시간 안에 끝나거나(P18 의 모양) · 그냥 틀리면 **Skill 측정** 단계에서 막힌다.

## 결과

- **P01** 을 고쳤다. 처음 줄을 1..M 으로 두는 조건을 더하고(M ≤ 400000), 큰 case 하나를
  맨 뒤에 넣었다. 실측: 정답(deque) 162ms, `pop(0)` 풀이는 TIME_LIMIT(2000ms 에서 끊김),
  앞 위치 번호 풀이는 ACCEPTED(132ms). 큰 case 때문에 `cases.json` 이 1.4MB 가 됐다 -
  Worker 가 호스트에서 읽으므로(judge-job 계약) 큐 행에는 들어가지 않는다
...
- **시간으로만 가른다.** 기계가 몇 배 빠르면 `pop(0)` 풀이가 제한 안에 들어올 수 있다.
  정답과의 차이를 크게(정답 1000ms 이하 요구, 실측 약 12배 이상) 두었지만 증명은 아니다
