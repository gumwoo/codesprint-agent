...
| `failedCaseRefs` 가 **Judge 가 실패시킨 case 를 포함하는가** | 실행되지도 않은 case 를 근거로 든 분석이 확정된다 |
| `secondaryMistakes` 에 `primaryMistake` 가 중복되지 않는가 | 같은 원인이 두 번 세어진다 |

### 2. §21-A 의 "case 특성 일치" 는 지금 **Judge 가 실패시킨 case 를 인용했는가** 로 구현한다

Judge 는 첫 실패에서 멈추므로(ADR-0005) 실패 case 는 하나로 특정된다. Reviewer 가
그 case 를 근거로 들지 않았다면, 그 분석은 **실제로 일어난 실패를 설명하고 있지 않다.**

이것이 §20 의 완전한 구현은 아니다. case 에 성격 태그가 붙으면 "경계 case 에서
실패했는데 BOUNDARY_CHECK 라고 했는가" 까지 볼 수 있다. **그때까지는 이 검사가
§21-A 의 전부다** - 더 강한 척하지 않는다. 문서와 코드가 어긋나는 것이 이 저장소에서
가장 자주 나온 결함이었다.

### 3. 확정 상태는 네 가지이고, 시스템이 정한다
...
- **확정되기까지 시간이 걸린다.** 처음 보는 실수는 대개 `POSSIBLE` 에서 멈춘다.
