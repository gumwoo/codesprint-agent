...
| P147 | W 를 0 부터 하나씩 늘리며 BFS 로 확인 | 무작위 격자에서는 ACCEPTED(498ms) - 답이 5777 인 뱀 복도 case 를 넣어 TIME_LIMIT, 정답 82ms. 이 case 는 "하나씩 늘리기" 만 떨어뜨린다 - 이분 탐색을 요구하는 근거는 아니다(아래) |
...
| P147(2 차) | 이분 탐색 없이 불 BFS · 최단 거리 BFS · 식 하나로 AC(81ms), 무작위 소형 격자 400 개에서 reference 와 불일치 0 | 문제를 그대로 두고 조합의 서술과 선수를 고쳤다(위 2). 답이 −1 · INF 에 몰려 있어 숫자 답 case 둘(50 · 103)을 더했다 |
...
- **SKILL_COMPOSITION 의 유일한 문제(P147)는 BFS 계열 안의 조합이다.** 여러 출발점 BFS 와 최단 거리 BFS 를 잇는 관찰이
  핵심이고, 정렬 · 누적 합 · 이분 탐색처럼 서로 다른 계열을 잇는 문제는 아직 없다. 생성한 섞인 문제 초안 다섯 중 넷이
  기존 문제와 같은 계산이었다(P142 · P143 · 작업 완료 시각 · 칠한 기둥). 대조가 없어 무엇을 이었는지는 SECONDARY
  Evidence 로만 남는다 - 섞인 문제를 하나 풀었다는 것이 어떤 조합이든 다룰 수 있다는 뜻은 아니다
