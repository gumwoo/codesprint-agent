...
| 문제 | 찾은 것 | 고친 것 |
| --- | --- | --- |
| P111 | 표 없이 1000 이하 소수 168 개로만 나누는 풀이가 AC(1.7 초) - 큰 case 의 수가 대부분 작은 소인수에서 일찍 끝났다 | 가장 작은 소인수가 큰 수(1000 넘는 소수 · 두 소인수가 300 이상인 반소수 · 그 제곱)로 채운 case, 시간 제한 2 초 → 1 초(정답 191ms). 그 풀이 · 메모 풀이 모두 TIME_LIMIT |
| 정의 | TRIE_PREFIX 가 "글자 단위 나무" 라는 방법을 적었는데 모든 접두사를 사전에 세는 풀이가 AC | 결과로 적었다. 나머지(체 · 가장 작은 소인수 · 이항 계수 · 스위핑)도 결과로 |
| P113 | 정답이 1194ms 로 채택 기준(1초)을 넘음 | N · Q ≤ 50000, 정답 255ms |

## 남는 위험

- **실패 함수는 해시로도 풀린다.** 접두사 해시와 이분 탐색으로 위치마다 공통 접두사 길이를 구하는 N log N 풀이가
  P109 에서 AC 다(1539ms). PREFIX_FUNCTION 의 Evidence 는 "경계와 접두사 등장을 효율적으로 구했다" 까지만 말한다
  (LIS 의 펜윅 트리 · 단조 스택의 sparse table 과 같은 처리)
- 정의상 받아들이는 풀이: Z 배열로 접두사 등장을 세는 풀이(P109), 역원을 질의마다 `pow` 로 구하는 풀이(P101),
  모든 접두사를 사전에 세는 풀이(P113 · P114) - 앞 둘은 같은 Skill 의 다른 구현이고, 뒤는 대조가 없는 TRIE_PREFIX 의
  정의(결과)를 만족한다
